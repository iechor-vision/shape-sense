...
use visioniechor::{Color, ColorName, CompoundPath, PathF64, PathI32, PointF64, PointI32, Spline};
...
impl Debugger for DrawUtil {
    ...
    fn draw_compound_path(&self, color: &Color, compound_path: &CompoundPath) {
        for path in compound_path.iter() {
            match path {
                visioniechor::CompoundPathElement::PathI32(path) => self.draw_path_i32(color, path),
                visioniechor::CompoundPathElement::PathF64(path) => self.draw_path_f64(color, path),
